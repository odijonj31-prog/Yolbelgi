// YoʻlBelgi boti v2: foydalanuvchilarni saqlash va reklama yuborish. Fayl joylashuvi: api/tg.js
...
const ADMIN = process.env.ADMIN_ID ? String(process.env.ADMIN_ID).trim() : '';
const ADMIN_PW = process.env.ADMIN_PW;
const SB_URL = process.env.SUPABASE_URL || 'https://ysdwwvmngaxjgorxptha.supabase.co';
const SB_KEY = process.env.SUPABASE_KEY || 'sb_publishable_GRZV0OHPkvEc0B8Ik1u0BA_oJ5VEdkL';
const BATCH_MS = +process.env.BATCH_MS || 6500;

const dbReady = () => !!(SB_URL && SB_KEY && ADMIN_PW);
...
const rpc = (fn, args) =>
  fetch(`${SB_URL}/rest/v1/rpc/${fn}`, {
    ...
    body: JSON.stringify(Object.assign({ p_pw: ADMIN_PW }, args || {}))
  }).then(async r => {
...
async function track(u) {
  if (!dbReady() || !u) return;
  try { await Promise.race([rpc('bot_track', { p_id: u.id, p_name: u.first_name || '' }), sleep(1500)]); }
...
async function sendQuiz(chat_id) {
  ...
  const r = await api('sendPoll', {
    chat_id, question: q.q, options: order.map(k => q.o[k]), type: 'quiz',
    correct_option_id: order.indexOf(q.a), explanation: q.e || undefined, is_anonymous: false
  });
  if (!r.ok) await say(chat_id, "Savolni yuborib boʻlmadi. Birozdan soʻng qayta urinib koʻring.");
}

/* ---------- Reklama (admin) ---------- */
async function sendStats(chat) {
  if (!dbReady()) return say(chat, 'Baza sozlanmagan: ADMIN_PW (Vercel) va SQL funksiyalarini tekshiring.');
  try {
    const s = await rpc('bot_stats', {});
    return say(chat, `👥 Foydalanuvchilar\nJami: ${s.total}\nFaol: ${s.active}\nBotni bloklaganlar: ${s.total - s.active}`);
  } catch (e) { return say(chat, 'Bazaga ulanib boʻlmadi: ' + String(e.message).slice(0, 150)); }
}

async function askSend(chat, m) {
  const r = m.reply_to_message;
  if (!r) return say(chat, 'Avval reklama postini shu yerga yozing, keyin oʻsha postga javob (reply) qilib /yubor deb yozing.');
  if (!dbReady()) return say(chat, 'Baza sozlanmagan: ADMIN_PW (Vercel) va SQL funksiyalarini tekshiring.');
  let n;
  try { n = (await rpc('bot_stats', {})).active; }
  catch (e) { return say(chat, 'Bazaga ulanib boʻlmadi: ' + String(e.message).slice(0, 150)); }
  return say(chat, `Yuqoridagi post ${n} ta foydalanuvchiga yuboriladi. Tasdiqlaysizmi?`, {
    reply_to_message_id: r.message_id,
    reply_markup: { inline_keyboard: [[
      { text: '✅ Ha, yuborish', callback_data: `bc:${r.message_id}:0:0:0` },
      { text: '❌ Bekor', callback_data: 'bc_no' }
    ]] }
  });
}

async function runBatch(chat, mid, after, sent, fail) {
  const t0 = Date.now();
  let done = false;
  while (Date.now() - t0 < BATCH_MS) {
    const t1 = Date.now();
    const rows = await rpc('bot_users_page', { p_after: after, p_limit: 28 });
    if (!rows || !rows.length) { done = true; break; }
    const dead = [];
    await Promise.all(rows.map(async r => {
      const body = { chat_id: r.id, from_chat_id: chat, message_id: mid };
      let res = await api('copyMessage', body);
      if (!res.ok && res.error_code === 429) {
        await sleep(((res.parameters && res.parameters.retry_after) || 1) * 1000);
        res = await api('copyMessage', body);
      }
      if (res.ok) sent++;
      else {
        fail++;
        if (res.error_code === 403 || (res.error_code === 400 && /chat not found|deactivated/i.test(res.description || ''))) dead.push(r.id);
      }
    }));
    after = rows[rows.length - 1].id;
    if (dead.length) { try { await rpc('bot_mark_blocked', { p_ids: dead }); } catch (e) {} }
    const wait = 1050 - (Date.now() - t1);
    if (wait > 0) await sleep(wait);
  }
  return { done, after, sent, fail };
}
...
async function onCallback(cb) {
  ...
  if (data.startsWith('bc')) {
    if (!isAdmin(cb.from.id)) return api('answerCallbackQuery', { callback_query_id: cb.id, text: 'Ruxsat yoʻq', show_alert: true });
    const mid_ = cb.message.message_id;
    if (data === 'bc_no') {
      await api('answerCallbackQuery', { callback_query_id: cb.id });
      return api('editMessageText', { chat_id: chat, message_id: mid_, text: 'Bekor qilindi.' });
    }
    const [, mid, after, sent, fail] = data.split(':').map((x, i) => i === 0 ? x : +x);
    await api('answerCallbackQuery', { callback_query_id: cb.id, text: 'Yuborilmoqda…' });
    await api('editMessageText', { chat_id: chat, message_id: mid_, text: '⏳ Yuborilmoqda… Kuting.' });
    let res;
    try { res = await runBatch(chat, mid, after, sent, fail); }
    catch (e) { return api('editMessageText', { chat_id: chat, message_id: mid_, text: 'Xato: ' + String(e.message).slice(0, 150) }); }
    if (res.done) {
      return api('editMessageText', { chat_id: chat, message_id: mid_, text: `✅ Yuborish tugadi.\nYetkazildi: ${res.sent}\nYetkazilmadi: ${res.fail}` });
    }
    return api('editMessageText', {
      chat_id: chat, message_id: mid_,
      text: `Yuborildi: ${res.sent}, xato: ${res.fail}.\nDavom ettirish uchun tugmani bosing 👇`,
      reply_markup: { inline_keyboard: [[{ text: '▶️ Davom ettirish', callback_data: `bc:${mid}:${res.after}:${res.sent}:${res.fail}` }]] }
    });
  }
  await api('answerCallbackQuery', { callback_query_id: cb.id });
  await track(cb.from);
  if (data === 'quiz' && cb.message) return sendQuiz(cb.message.chat.id);
}
...
async function onMessage(m, appUrl) {
  ...
  const admin = isAdmin(m.from && m.from.id);
  await track(m.from);

  if (text.startsWith('/id')) return say(chat, `Sizning Telegram ID: ${m.from.id}`);

  if (admin) {
    if (text.startsWith('/stat')) return sendStats(chat);
    if (text.startsWith('/yubor')) return askSend(chat, m);
    if (text.startsWith('/reklama')) {
      return say(chat, '📣 Reklama yuborish\n\n1) Reklama postini shu chatga yozing (matn, rasm yoki video, sarlavha bilan).\n2) Oʻsha postga javob (reply) qilib /yubor deb yozing.\n3) Tasdiqlash tugmasini bosing.\n\n/stat: foydalanuvchilar soni');
    }
  }
  ...
  if (admin) return; // admin reklama postini yozayotgan boʻlishi mumkin: javob bermaymiz
...
async function onMyChatMember(u) {
  if (!u.chat || u.chat.type !== 'private' || !dbReady()) return;
  const st = u.new_chat_member && u.new_chat_member.status;
  try {
    if (st === 'kicked') await rpc('bot_mark_blocked', { p_ids: [u.chat.id] });
    else if (st === 'member') await rpc('bot_track', { p_id: u.chat.id, p_name: (u.from && u.from.first_name) || '' });
  } catch (e) { console.error('mcm', e.message); }
}

module.exports = async (req, res) => {
  res.setHeader('Content-Type', 'text/plain; charset=utf-8');
  if (!TOKEN) return res.status(500).send('BOT_TOKEN topilmadi. Vercel > Settings > Environment Variables.');

  const host = process.env.VERCEL_PROJECT_PRODUCTION_URL || req.headers['x-forwarded-host'] || req.headers.host;
  const appUrl = `https://${host}`;

  // Brauzerda ochilsa: botni sozlaydi va holatni koʻrsatadi
  if (req.method === 'GET') {
    ...
    if (ADMIN) {
      await api('setMyCommands', {
        ...
        commands: [
          { command: 'reklama', description: 'Reklama yuborish yoʻriqnomasi' },
          { command: 'yubor', description: 'Postni hammaga yuborish (reply)' },
          { command: 'stat', description: 'Foydalanuvchilar soni' },
          { command: 'start', description: 'Botni boshlash' },
          { command: 'savol', description: 'Tasodifiy savol' }
        ]
      });
    ...
    let dbLine = '❌ Baza: ADMIN_PW kiritilmagan';
    if (dbReady()) {
      try { const s = await rpc('bot_stats', {}); dbLine = `✅ Baza ulangan. Foydalanuvchilar: ${s.total}`; }
      catch (e) { dbLine = '❌ Baza xatosi: ' + String(e.message).slice(0, 120); }
    }
    return res.status(200).send([
      wh.ok ? '✅ Bot sozlandi.' : `❌ Webhook xatosi: ${wh.description || 'nomaʼlum'}`,
      `Ilova manzili: ${appUrl}`,
      ADMIN ? '✅ Admin ID kiritilgan.' : '❌ ADMIN_ID kiritilmagan.',
      dbLine
    ].join('\n'));
  }
  ...
  try {
    ...
    else if (u.my_chat_member) await onMyChatMember(u.my_chat_member);
